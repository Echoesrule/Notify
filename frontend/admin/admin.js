...
async function loadSectionData(sectionId) {
    ...
    try {
        switch(sectionId) {
            case 'home':
                await loadDashboardStats(headers);
                break;
            case 'users':
                await loadUsers(headers);
                break;
            case 'institutions':
                await loadInstitutions(headers);
                break;
            case 'schools':
                await loadSchools(headers);
                break;
            case 'courses':
                ...
                break;
            case 'units':
                await loadUnits(headers);
                break;
            case 'notes':
                await loadNotes(headers);
                break;
            case 'updates':
                await loadUpdates(headers);
                break;
        }
    } catch (err) {
...
async function loadDashboardStats(headers) {
    try {
        ...
        document.getElementById('totalInstitutions').textContent = institutions.length || 0;
        document.getElementById('totalNotes').textContent = notes.length || 0;
        document.getElementById('totalUpdates').textContent = updates.length || 0;
...
function filterUsers() {
    ...
    rows.forEach(row => {
        ...
        const showRole = !roleFilter || row.textContent.includes(roleFilter);
...
function loadUserDataForSettings() {
    ...
    if (updatePasswordBtn) {
        updatePasswordBtn.onclick = handleAdminPasswordChange;
    }
}
